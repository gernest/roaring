...
import (
	...
	"github.com/gernest/roaring/generator"
...
func TestBitmap_BitmapCountRangeEdgeCase(t *testing.T) {
	s := uint64(2009 * shardwidth.ShardWidth)
	e := uint64(2010 * shardwidth.ShardWidth)

	start := s + (39314024 % shardwidth.ShardWidth)
...
func TestBitmap_Difference2(t *testing.T) {
	bm0 := roaring.NewFileBitmap(0, 1, 2, 131072, 262144, shardwidth.ShardWidth+5, shardwidth.ShardWidth+7)
	bm1 := roaring.NewFileBitmap(2, 3, 100000, 262144, 2*shardwidth.ShardWidth+1)
	result := bm0.Difference(bm1)
	if !reflect.DeepEqual(result.Slice(), []uint64{0, 1, 131072, shardwidth.ShardWidth + 5, shardwidth.ShardWidth + 7}) {
			...
			for n := 0; n < b.N; n++ {
				...
				for row := uint64(1); row < NumRows; row++ {
					for col := uint64(1); col < NumColums; col++ {
						_, _ = bm.Add(row*shardwidth.ShardWidth + (col * MaxContainerVal))
					}
				}
			}
			...
			for n := 0; n < b.N; n++ {
				...
				for col := uint64(1); col < NumColums; col++ {
					for row := uint64(1); row < middle; row++ {
						_, _ = bm.Add(row*shardwidth.ShardWidth + (col * MaxContainerVal))
						_, _ = bm.Add((NumRows-row)*shardwidth.ShardWidth + (col * MaxContainerVal))
					}
				}
			}
		})
	...
	for i, bmMaker := range bmFuncs {
		b.Run(bmFuncNames[i], func(b *testing.B) {
			for n := 0; n < b.N; n++ {
				...
				for col := uint64(1); col < NumColums; col++ {
					for row := uint64(1); row <= middle; row++ {
						_, _ = bm.Add((middle+row)*shardwidth.ShardWidth + (col * MaxContainerVal))
						_, _ = bm.Add((middle-row)*shardwidth.ShardWidth + (col * MaxContainerVal))
					}
				}
			}
...
func BenchmarkSliceAscending(b *testing.B) {
	for n := 0; n < b.N; n++ {
		bm := roaring.NewFileBitmap()
		for col := uint64(0); col < shardwidth.ShardWidth; col++ {
			_, _ = bm.Add(col)
		}
	}
}
...
func BenchmarkSliceDescending(b *testing.B) {
	for n := 0; n < b.N; n++ {
		bm := roaring.NewFileBitmap()
		for col := uint64(shardwidth.ShardWidth); col > uint64(0); col-- {
			_, _ = bm.Add(col)
		}
		_, _ = bm.Add(0)
	}
...
func BenchmarkSliceAscendingStriped(b *testing.B) {
	for n := 0; n < b.N; n++ {
		bm := roaring.NewFileBitmap()
		l := uint64(shardwidth.ShardWidth / 8)
